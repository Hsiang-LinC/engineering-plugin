...
3. **Labels** — actor / work-type / gate classifications ("none" is
   valid). When workflow skills are detected, this section also carries
   their canonical triage roles mapped to concrete tracker labels/states:
   category roles (`bug`, `enhancement`) and state roles
   (`needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`,
   `wontfix`). Reconcile these with dispatch/actor labels per the
...
{workflow skills detected: include category roles `bug`, `enhancement` and
state roles `needs-triage`, `needs-info`, `ready-for-agent`,
`ready-for-human`, `wontfix`, mapped to local entry fields or labels}
...
- triage category: {`bug`, `enhancement` when workflow skills detected,
  mapped per the substitution rule | omit}
- triage state: {`needs-triage`, `needs-info`, `ready-for-agent`,
  `ready-for-human`, `wontfix` when workflow skills detected, mapped per
  the substitution rule | omit}
