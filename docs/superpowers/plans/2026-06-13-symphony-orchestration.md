...
- [ ] **Step 1b: Create a README stub** (REQUIRED before install)

`pyproject.toml` declares `readme = "README.md"`, and hatchling validates that
file during metadata generation **even for editable installs** — a missing
README fails the install. Create a stub now (Task 1.7 expands it):

```markdown
# codex-symphony

Issue-tracker-driven coding-agent orchestration engine: scheduler + review /
accept / janitor / reconcile. Extracted from trading-advisor's extended Symphony.

Per-repo config (WORKFLOW.md, REVIEW.md, .env) is generated by the
`setup-symphony-orchestration` skill. Install: `pip install git+<repo-url>`.

> Fuller docs (CLI surface, contract) land in the publish task.
```
...
git commit -m "build: add codex-symphony package metadata + README stub"
```
(Add a `.gitignore` with `.venv/`, `__pycache__/`, `*.pyc` first; never commit `.venv/`.)
